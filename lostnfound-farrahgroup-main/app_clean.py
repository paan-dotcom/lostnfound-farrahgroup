...
class Item(db.Model):
    ...
    image_file = db.Column(db.String(200)) # Ensure this matches the HTML
...
def home():
    ...
    # Fetch the items (the 'items' objects now contain the 'image_file' data)
...
# 1. Define allowed extensions at the top of your file
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif', 'webp'}
...
def report():
    if request.method == 'POST':
        # Initialize the filename as None in case no file is uploaded
        image_filename = None
        ...
        if 'file' in request.files:
            ...
            # 3. Validate the file exists and has an allowed image extension
            if file and file.filename != '':
                if allowed_file(file.filename):
                    # Secure the filename to prevent directory traversal attacks
                    filename = secure_filename(file.filename)
                    # Add a unique prefix or timestamp if you want to avoid overwriting files
                    file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                    image_filename = filename
                else:
                    flash('Error: Only image files (jpg, png, jpeg, gif, webp) are allowed!')
        ...
        new_item = Item(
            ...
            image_file=image_filename  # 4. Save the filename to the database
        )
